...
function NotFound() {
  return (
    <div className="grid min-h-screen place-items-center px-6 text-center">
      <div>
        <Logo />
        <h1 className="mt-6 font-display text-5xl font-extrabold text-fg">
          404
        </h1>
        <p className="mt-2 text-fg-dim">This page couldn't be found.</p>
        <Link to="/">
...
function Protected() {
  ...
  if (!authReady)
    return (
      <div className="grid min-h-screen place-items-center text-fg-dim">
        Loading workspace…
      </div>
    )
  return session ? <AppLayout /> : <Navigate to="/sign-in" replace />
}
      ...
      <AppProvider>
        <Routes>
          ...
          <Route path="/app" element={<Protected />}>
            ...
            <Route
              path="investigations/quick"
              element={<Navigate to="/app/investigations/new" replace />}
            />
            <Route
