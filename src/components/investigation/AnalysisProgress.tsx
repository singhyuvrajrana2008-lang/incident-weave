...
export function AnalysisProgress({ analysisRunId, onComplete }: { analysisRunId: string; onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      } catch (e) { if (active) setError(e instanceof Error ? e.message : "Unable to read analysis status."); }
    };
    void poll();
    return () => { active = false; };
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      {error ? <div className="flex items-start gap-2 rounded-md border border-crimson/30 bg-crimson/10 p-4 text-sm text-crimson"><AlertCircle className="size-4 shrink-0" />{error}</div> : <>
