        ...
        <div className="fixed inset-0 z-[70] flex items-start justify-center p-4 pt-[12vh]">
          ...
          >
            <div className="flex items-center gap-3 border-b border-line px-4">
              ...
              />
              <kbd className="rounded-xs border border-line-2 bg-surface px-1.5 py-0.5 font-mono text-[10px] text-fg-dim">
                ESC
              </kbd>
            </div>
            <div className="max-h-[52vh] overflow-y-auto scroll-thin p-2">
              {results.length === 0 ? (
                <div className="px-3 py-10 text-center text-sm text-fg-dim">
                  No results for "{q}"
                </div>
              ) : (
                grouped.map(([group, items]) => (
                  <div key={group} className="mb-1">
                    <div className="px-2 py-1.5 font-mono text-[10px] uppercase tracking-wider text-fg-faint">
                      {group}
                    </div>
                    {items.map((r) => {
                      flatIndex++
                      ...
                      return (
                        ...
                        >
                          <r.icon
                            className={cn(
                              "size-4 shrink-0",
                              active === idx ? "text-accent" : "text-fg-dim",
                            )}
                          />
                          ...
                          {active === idx && (
                            <CornerDownLeft className="size-3.5 text-fg-dim" />
                          )}
                        </button>
                      )
                    })}
