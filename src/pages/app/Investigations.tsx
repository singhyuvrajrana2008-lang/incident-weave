...
export default function Investigations() {
  ...
  useEffect(() => { investigationService.list().then((rows) => setData([DEMO_INVESTIGATION, ...rows])); }, []);
  ...
  function openInvestigation(inv: Investigation) {
    if (inv.id === DEMO_INVESTIGATION.id) {
      toast({ title: "Judge demo investigation", kind: "info", desc: "This presentation case is populated with stable demo findings." });
      return;
    }
    nav(`/app/investigations/${inv.id}`);
  }
