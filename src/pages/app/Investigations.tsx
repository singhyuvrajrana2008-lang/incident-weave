...
export default function Investigations() {
  ...
  useEffect(() => { investigationService.list().then(setData); }, []);
  ...
  return (
    <Page>
      <PageHeader
        ...
        actions={<Link to="/app/investigations/new"><Button variant="primary" icon={<PlusCircle className="size-4" />}>New Investigation</Button></Link>}
      />
      ...
      {!data ? (
        ...
        <Panel>
          <EmptyState
            ...
            title={q || status !== "all" ? "No matching investigations" : "No investigations yet"}
            description={q || status !== "all" ? "Try adjusting your search or filters." : "Create your first investigation to start reconstructing incidents."}
            action={<Link to="/app/investigations/new"><Button variant="primary" icon={<PlusCircle className="size-4" />}>New Investigation</Button></Link>}
          />
        </Panel>
      ) : (
              ...
              <thead>
                <tr className="border-b border-line text-left font-mono text-[10px] uppercase tracking-wider text-fg-faint">
                  {["Case", "Created", "Updated", "Evidence", "Events", "Conflicts", "Unknowns", "Status"].map((h, i) => (
                    <th key={h} className={`px-4 py-2.5 font-normal ${i >= 3 && i <= 6 ? "text-right" : ""}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {rows.map((inv) => (
                  <tr key={inv.id} onClick={() => nav(`/app/investigations/${inv.id}`)} className="group cursor-pointer border-b border-line/60 transition-colors last:border-0 hover:bg-surface-2">
                    ...
                    <td className="px-4 py-3 text-right font-mono tabular"><span className={inv.contradictionCount ? "text-crimson" : "text-fg-dim"}>{inv.contradictionCount}</span></td>
                    <td className="px-4 py-3 text-right font-mono tabular"><span className={inv.unknownCount ? "text-amber" : "text-fg-dim"}>{inv.unknownCount}</span></td>
