...
export default function InvestigationWorkspace() {
  ...
  async function exportReport() {
    ...
    try {
      ...
      const evidenceRows = inv.evidence
        .map((e) => "<tr><td>" + escapeHtml(e.filename) + "</td><td>" + escapeHtml(e.type.toUpperCase()) + "</td><td>" + escapeHtml(e.size) + "</td><td>" + escapeHtml(e.status) + "</td><td>" + escapeHtml(e.relevantTime) + "</td></tr>")
        .join("");

      const timelineRows = inv.events
        .map((e) => "<tr><td>" + escapeHtml(e.date + " · " + e.time) + "</td><td>" + escapeHtml(e.title) + "</td><td>" + escapeHtml(e.description) + "</td><td>" + escapeHtml(e.confidence) + "</td></tr>")
        .join("");
      ...
      if (!reportWindow) throw new Error("The report window was blocked. Allow pop-ups for IncidentWeave and try again.");
      reportWindow.document.open();
      reportWindow.document.write(html);
      reportWindow.document.close();
      reportWindow.focus();
      window.setTimeout(() => reportWindow.print(), 250);
      toast({ title: "Report ready", kind: "success", desc: "Use Save as PDF in the print dialog." });
    } catch (reason) {
      toast({ title: "Export failed", kind: "danger", desc: reason instanceof Error ? reason.message : "Unable to prepare the report." });
    } finally {
      setExporting(false);
    }
  }
        ...
        <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              size="sm"
              loading={exporting}
              disabled={exporting || addingEvidence || rerunStarting || !!rerunRunId}
