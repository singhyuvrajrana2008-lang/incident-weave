...
export const authService = {
  ...
  async signIn(email: string, password: string) { const client = requireSupabase(); const { error } = await client.auth.signInWithPassword({ email, password }); if (error) throw new Error(error.message); return (await this.current())!; },
  async signUp(name: string, email: string, password: string) { const client = requireSupabase(); const { data, error } = await client.auth.signUp({ email, password, options: { data: { full_name: name } } }); if (error) throw new Error(error.message); if (!data.session) throw new Error("Account created. Check your email to confirm your account before signing in."); return (await this.current())!; },
  async resetPassword(email: string) { const client = requireSupabase(); const { error } = await client.auth.resetPasswordForEmail(email, { redirectTo: `${window.location.origin}/sign-in` }); if (error) throw new Error(error.message); },
  async signOut() { const { error } = await requireSupabase().auth.signOut(); if (error) throw new Error(error.message); },
  async updateProfile(patch: Partial<Session>) { const session = await this.current(); if (!session) return; const values: Record<string, string> = { updated_at: new Date().toISOString() }; if (patch.name !== undefined) values.full_name = patch.name; if (patch.role !== undefined) values.role = patch.role; if (patch.workspace !== undefined) values.workspace = patch.workspace; const { error } = await requireSupabase().from("profiles").update(values).eq("id", session.id); if (error) throw new Error(error.message); },
};

function toEvidence(row: Record<string, unknown>): Evidence { const extraction = (row.evidence_extractions as Record<string, unknown>[] | undefined)?.[0]; return { id: String(row.id), filename: String(row.filename), type: fileType(row.mime_type as string | null, String(row.filename)), size: `${(Number(row.size_bytes ?? 0) / 1e6).toFixed(1)} MB`, uploadedAt: relative(row.created_at as string), relevantTime: String(extraction?.relevant_time ?? "—"), status: (row.status as Evidence["status"]) ?? "processing", confidence: (extraction?.confidence as Evidence["confidence"]) ?? "medium", sourceId: String(row.id).slice(0, 8), observations: Array.isArray(extraction?.observations) ? extraction!.observations as string[] : [], relatedEvents: [], relatedContradictions: [], notes: extraction?.extracted_text as string | undefined }; }
...
async function audit(action: string, investigationId: string | null, metadata: Record<string, unknown> = {}) { const client = requireSupabase(); const user = (await client.auth.getUser()).data.user; if (user) await client.from("audit_log").insert({ user_id: user.id, investigation_id: investigationId, action, metadata }); }
