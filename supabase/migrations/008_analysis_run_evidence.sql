...
create policy "analysis run evidence through investigation" on public.analysis_run_evidence
  for all using (exists (select 1 from public.analysis_runs ar where ar.id = analysis_run_id and public.is_owner(ar.investigation_id)))
  with check (exists (select 1 from public.analysis_runs ar where ar.id = analysis_run_id and public.is_owner(ar.investigation_id)));
